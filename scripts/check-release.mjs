...
export function assertReleaseInputs(policy, manifest, lock, env) {
  ...
  if (env.GITHUB_EVENT_NAME === 'workflow_dispatch') {
    assert.equal(env.UI_RELEASE_TAG, expectedTag);
  }
  assert.equal(policy.activationVariable, 'UI_NPM_PUBLISH_ENABLED');
...
if (process.argv[1] && resolve(process.argv[1]) === fileURLToPath(import.meta.url)) {
  const releaseRoot = realpathSync(resolve(authorityRoot, process.env.UI_RELEASE_ROOT ?? '.'));
