<script lang="ts">
  ...
  import WorkspaceRootSplitSegmentRegion from './WorkspaceRootSplitSegmentRegion.svelte';
  import WorkspaceSplitFrame from './WorkspaceSplitFrame.svelte';
  import type { ResizeDeletePreview } from './resizeBoundaryPreview';
  import {
    buildSplitGridStyle,
    type FlattenedSplitChain,
    type FlattenedSplitHandle
  } from './splitChain';
  import {
  ...
  export let rootSplitChain: FlattenedSplitChain;
  ...
  export let rootResizeDragSession: WorkspaceNodeResizeDragSession | null = null;
  ...
  $: rootRenderSegments = resolveWorkspaceSplitResizePreviewChain({
    splitChain: rootSplitChain,
    session: rootResizeDragSession
  });
</script>

<WorkspaceSplitFrame
  bind:splitElement={rootSplitElement}
  splitId={rootSplitId}
  orientation={rootSplitChain.orientation}
  splitStyle={buildSplitGridStyle(rootSplitChain.orientation, rootRenderSegments)}
  className="workspace-view__root-split"
>
  {#each rootSplitChain.segments as segment, index (segment.id)}
    <WorkspaceRootSplitSegmentRegion
      {segment}
      handle={rootSplitChain.handles[index] ?? null}
      {rootSplitId}
      boundaryIndex={index}
      orientation={rootSplitChain.orientation}
      {workspace}
